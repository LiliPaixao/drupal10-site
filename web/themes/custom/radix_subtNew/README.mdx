...
radix_subtNew theme uses [Webpack](https://webpack.js.org) to compile and
bundle SASS and JS.
...
Go to the root of radix_subtNew theme and run the following commands:
`nvm use`
...
`drush en radix_subtNew -y`
`drush config-set system.theme default radix_subtNew -y`
